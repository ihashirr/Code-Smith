		...
		<div className="flex flex-col items-center min-h-screen bg-gradient-to-r from-blue-400 to-purple-600 p-6">
		  ...
		  <AnimatePresence>
			{showModal && (
			  ...
			  >
				<motion.div
				  className="bg-white rounded-lg shadow-2xl p-8 w-full max-w-md transform transition-transform hover:scale-105 duration-200"
				  initial={{ scale: 0.8 }}
				  animate={{ scale: 1 }}
				  exit={{ scale: 0.8 }}
				>
					  ...
					  {selectedFormData.monthlyFees && (
						<div>
						  ...
						  {selectedFormData.monthlyFees.map((month, index) => (
							<p key={index}>{month.name}: £{month.fee}</p>
						  ))}
						</div>
					  )}
